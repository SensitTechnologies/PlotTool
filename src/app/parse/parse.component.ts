...
//  private _values2 = [];
  ...
  firstDropDownChanged(val: any) {
    ...
    const obj = this.values1[val];//obj = the item that is in val index of this._values1 (has "id" and "val" entries)
  ...
  readThis(inputValue: any): void { //Choose file uses this
    ...
    myReader.onloadend = (e) => {//This event is triggered each time the reading operation is successfully completed.
      ...
      this.fileString = myReader.result; //myReader.result is a String made up of entire uploaded CSV file
     console.log(this.fileString)
      for(var cut=0;this.fileString[cut]!="\n"; cut++){ //Grab titles (letter by letter) until the first newline. (Titles will always be the first line)
        this.tempTitles = this.fileString.slice(0,cut); //Temporary tile array, read only
      }  
     ...
     // console.log("Title Array: "+this.titleArray)
      ...
      for(var counter=0;counter<this.titleArray.length;counter++){ //Xvalue drop down options   
         this.values1.push({label:this.titleArray[counter], value:counter})  //push all titles into an array using object notation (label: title,value: index)
      }

      console.log("v1"+JSON.stringify(this.values1));


      this.parsedFileString=this.fileString.split(/\r\n|\n/); //Entire loaded in file //Remove /n, "this.parsedFileString" = a list of entire loaded in CSV file
   ...
   ///////   console.log("sF2"+JSON.stringify(scatFinal));
    ...
    dataGrabber(){  //send data to chart, plot ONLY after FILE SELECTION
      ...
      this.dataProvider.plotData=this.dataProvider.data;  //this.dataProvider.data is an array of objects, chart needs array of objects
